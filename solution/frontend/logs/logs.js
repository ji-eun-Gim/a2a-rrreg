...
function setupControls() {
  ...
  const sourceButtons = document.querySelectorAll('[data-log-source]');
  sourceButtons.forEach((button) => {
    button.addEventListener('click', () => {
      sourceButtons.forEach((btn) => btn.classList.toggle('active', btn === button));
      filters.source = button.dataset.logSource || 'all';
      renderResults();
    });
  });
...
function applyFilters(log) {
  if (filters.source !== 'all' && getLogSource(log) !== filters.source) {
    return false;
  }
...
function renderTable(logs) {
  ...
  if (!logs || logs.length === 0) {
    ...
    row.innerHTML = '<td colspan="7" class="empty-state">조건에 맞는 로그가 없습니다.</td>';
  ...
  logs.forEach((log) => {
    ...
    const ipCell = clone.querySelector('.log-ip');
    ...
    if (ipCell) {
      ipCell.textContent = log.client_ip || log.extra?.clientIp || '없음';
    }
    if (verdictCell) {
